## Hi, I'm Michael Ellis

Staff Data Scientist specializing in **Bayesian inference**, **time series forecasting**, and **conversational AI**. I build production ML systems, publish packages, and contribute to open source.

### Currently

- Contributing new forecast accuracy metrics to [**sktime**](https://github.com/sktime/sktime)
- Adding PSIS k-hat diagnostics to [**NumPyro**](https://github.com/pyro-ppl/numpyro)
- Building transformer-based language models with PyTorch ([**LMT**](https://github.com/michaelellis003/LMT))

### Tech Stack

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JAX](https://img.shields.io/badge/JAX-A435F0?style=flat&logo=google&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![NumPyro](https://img.shields.io/badge/NumPyro-0D6EFD?style=flat)
![R](https://img.shields.io/badge/R-276DC3?style=flat&logo=r&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=cplusplus&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)

### Featured Work

- [**LMT**](https://github.com/michaelellis003/LMT) — PyTorch implementation of transformer-based language models (GPT) for pretraining and fine-tuning
- [**uv-python-template**](https://github.com/michaelellis003/uv-python-template) — Production-ready Python package template with uv, ruff, and GitHub Actions
- [**gaussian-process**](https://github.com/michaelellis003/gaussian-process) — Bayesian spatial inference with Gaussian processes in R
- [**variational-inference**](https://github.com/michaelellis003/variational-inference) — Variational Bayes and CAVI algorithms for approximate inference

### Open Source Contributions

- [**sktime**](https://github.com/sktime/sktime) — new forecast accuracy metrics for the time series ML framework
- [**NumPyro**](https://github.com/pyro-ppl/numpyro) — PSIS k-hat diagnostics for variational inference
- [**HuggingFace Evaluate**](https://github.com/huggingface/evaluate) — label distribution entropy metric

### Connect

[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:michaelellis003@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/michaelellis003)
[![Website](https://img.shields.io/badge/mlellis.com-228B22?style=for-the-badge&logo=safari&logoColor=white)](https://www.mlellis.com)
